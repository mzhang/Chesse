...
void ChessDrawing::draw_axes(Screen &screen) {
  ...
  for (int i = 0; i < 8; i++) {
    screen.draw_text(20, (7 - i) * square_size + y_offset + 20, 50, 50,
                     std::to_string((i + 1)), 20, WHITE);
  }
  // Write column numbers (chess)
  for (int i = 0; i < 8; i++) {
    screen.draw_text(i * square_size + x_offset + 25, screen.getHeight() - 40,
                     50, 50, std::string(1, (char)('A' + i)), 20, WHITE);
  }
}
...
void ChessDrawing::clear_square(Screen &screen, int x, int y) {
  ...
  if ((x + y) % 2 == 0) {
    screen.draw_rect(x * square_size + x_offset, y * square_size + y_offset,
...
void ChessDrawing::draw_potential_moves(Screen &screen,
                                        const std::vector<Move> &moves) {
  // Draw a small square on each square corresponding to move.to in each move in
  // moves
  int indicator_size = square_size / 4;
  for (const Move &move : moves) {
    for (const Position &pos : move.to) {
      screen.draw_rect(pos.x * square_size + x_offset,
                       pos.y * square_size + y_offset, indicator_size,
                       indicator_size, WHITE);
    }
  }
}
...
void ChessDrawing::draw_valid_position(Screen &screen, int x, int y) {
  ...
  screen.draw_image(x * square_size + x_offset + 25,
                    (7 - y) * square_size + y_offset + 25, 20, 20, filename);
}

void ChessDrawing::draw_chesspiece(Screen &screen, int x, int y,
                                   PieceType piece, PlayerColor owner) {
  ...
  screen.draw_image(x * square_size + piece_x_offset,
                    (7 - y) * square_size + piece_y_offset, square_size,
