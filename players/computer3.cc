...
bool Computer3::avoidsCapture(const GameState &state, const Move &move) {
  int attackedCount =
      state.numberOfTilesAttacked(playerColor, move.capturePositions);

  GameState newState{state};
  CompletedMove cm = newState.makeMove(move, isHeadless());
  int newAttackedCount =
      newState.numberOfTilesAttacked(playerColor, move.capturePositions);

  // cout << "Number of pieces attacked before: " << attackedCount << "Number of
  // pieces attacked after: " << newAttackedCount << endl;
