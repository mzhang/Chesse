...
PawnRules::PawnRules(unique_ptr<Moveable> component, int enpassantRank,
                     int promotionRank, int promotedMaxSteps)
    : Decorator{std::move(component)}, enpassantRank{enpassantRank},
      promotionRank{promotionRank}, isPromoted{false}, promotedMaxSteps{
                                                           promotedMaxSteps} {}
PawnRules::PawnRules(const PawnRules &o)
    : Decorator{o}, enpassantRank{o.enpassantRank},
      promotionRank{o.promotionRank}, isPromoted{o.isPromoted},
      promotedMaxSteps{o.promotedMaxSteps} {}
...
vector<Move> PawnRules::getValidMoves(const GameState &g,
  ...
  if (g.isInBounds(forwards) && g.isInBounds(forwards2) &&
      g.isEmpty(forwards) && g.isEmpty(forwards2) &&
      Decorator::getMovedCount() == 0) {
  ...
  if (g.isInBounds(leftDiag) && !g.isOwner(leftDiag, player) &&
      !g.isEmpty(leftDiag)) {
    moves.push_back(Move{currentPos, leftDiag, leftDiag});
  }
  if (g.isInBounds(rightDiag) && !g.isOwner(rightDiag, player) &&
      !g.isEmpty(rightDiag)) {
  ...
  if (currentPos.y + 1 == enpassantRank && g.isInBounds(left) &&
      !g.isOwner(left, player) && !g.isEmpty(left) &&
      g.getPieceType(left) == PieceType::PAWN && g.getMovedCount(left) == 1) {
    bool lastMovedPawn = false;
    vector<Position> lastMoveTo = g.lastMove.to;
    for (int i = 0; i < (int)lastMoveTo.size(); i++) {
      if (lastMoveTo[i] == left &&
          g.getPieceType(lastMoveTo[i]) == PieceType::PAWN) {
        lastMovedPawn = true;
        break;
      }
    }
  ...
  if (currentPos.y + 1 == enpassantRank && g.isInBounds(right) &&
      !g.isOwner(right, player) && !g.isEmpty(right) &&
      g.getPieceType(right) == PieceType::PAWN && g.getMovedCount(right) == 1) {
    bool lastMovedPawn = false;
    vector<Position> lastMoveTo = g.lastMove.to;
    for (int i = 0; i < (int)lastMoveTo.size(); i++) {
      if (lastMoveTo[i] == right &&
          g.getPieceType(lastMoveTo[i]) == PieceType::PAWN) {
        lastMovedPawn = true;
        break;
      }
...
void PawnRules::promote(bool headless) {
  ...
  if (headless) {
    Decorator::setComponent(
        PieceFactory::createPiece(currentPos, PieceType::QUEEN, player,
                                  promotedMaxSteps, promotedMaxSteps));
    return;
  }

  vector<PieceType> validPromotions = {PieceType::QUEEN, PieceType::ROOK,
  ...
  pair<PieceType, PlayerColor> promotion =
      PieceTypeUtils::fromString(pieceType);

  while (find(validPromotions.begin(), validPromotions.end(),
              promotion.first) == validPromotions.end() ||
         promotion.second != player) {
    cout << "Invalid piece type. Try again." << endl;
    cin >> pieceType;
    promotion = PieceTypeUtils::fromString(pieceType);
  }

  Decorator::setComponent(PieceFactory::createPiece(
      currentPos, promotion.first, player, promotedMaxSteps, promotedMaxSteps));
}
