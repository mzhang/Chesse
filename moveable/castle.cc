...
Castle::Castle(unique_ptr<Moveable> component,
               vector<Position> partnerPositions)
    : Decorator{std::move(component)}, partnerPositions{partnerPositions} {}
Castle::Castle(const Castle &o)
    : Decorator{o}, partnerPositions{o.partnerPositions} {}

vector<Position> Castle::getAttackedTiles(const GameState &g, bool) const {
  return Decorator::getAttackedTiles(g, false);
}

vector<Move> Castle::getValidMoves(const GameState &g,
  ...
  for (auto &castlePartner : partnerPositions) {
    ...
    if (!g.isInBounds(castlePartner) || !g.isOwner(castlePartner, player) ||
        (g.getMovedCount(castlePartner) != 0) ||
    ...
    if (g.isInBounds(finalPos) && g.isEmpty(finalPos)) {
      moves.push_back(Move{vector<Position>{currentPos, castlePartner},
                           vector<Position>{finalPos, partnerPos}});
    }
  }
