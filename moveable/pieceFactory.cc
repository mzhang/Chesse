...
unique_ptr<Moveable> PieceFactory::createPiece(const Position &p,
                                               PieceType type,
                                               PlayerColor owner, int width,
                                               int height) {
  ...
  case PieceType::KING:
    ...
    piece = make_unique<Castle>(
        std::move(piece),
        vector<Position>{Position{0, p.y}, Position{width - 1, p.y}});
    break;
