...
Game::Game(int boardWidth, int boardHeight, bool useDisplay,
           unordered_map<PlayerColor, string> playerStrings)
    : state{boardWidth, boardHeight} {
...
PlayerColor Game::play() {
  ...
  while (cin >> cmd) {
    if (cmd == "setup") {
      ...
      if (gameEnded.first) {
        cout << (gameEnded.second == PlayerColor::NONE ? "Stalemate! "
                                                       : "Checkmate! ");
      ...
      // make sure there's no malformed moves. These should never happen, but
      // just in case.
      if (move.from.size() == 0 || move.to.size() == 0 ||
          move.from.size() != move.to.size() || move.from == move.to ||
          !state.isValidMove(move)) {
        throw runtime_error("Malformed move after nextMove called. Was there "
                            "some malformed move in getValidMoves?");
      }
...
unique_ptr<Player> Game::getPlayer(const string &playerName,
