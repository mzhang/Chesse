...
Board::Board(const Board &o) : width{o.width}, height{o.height} {
  ...
  // cout << "DEBUG: Board copy constructor. Make sure this is intended" <<
  // endl;

  for (int i = 0; i < height; i++) {
    for (int j = 0; j < width; j++) {
      if (o.board[i][j]) { // if there is a piece at this position (aka not
                           // nullptr)
...
CompletedMove Board::makeMove(Move move, bool headless) {
  ...
  vector<unique_ptr<Moveable>>
      from; // we need to store the pieces that will move since we may have a
            // second piece overlapping
  ...
  for (int i = 0; i < (int)move.capturePositions.size(); ++i) {
    if (!isEmpty(move.capturePositions[i])) {
      capturedPieces.emplace_back(make_pair(
          move.capturePositions[i], popPiece(move.capturePositions[i])));
    }
  }
...
void Board::undoMove(CompletedMove &&m) {
  ...
  for (size_t i = 0; i < m.move.from.size(); i++) {
    ...
    board[m.move.from[i].y][m.move.from[i].x]->onMove(m.move, m.move.from[i],
                                                      true, -1);
  }
  ...
  for (size_t i = 0; i < m.capturedPieces.size(); i++) {
    ...
    board[m.capturedPieces[i].first.y][m.capturedPieces[i].first.x]->onMove(
        m.move, m.capturedPieces[i].first, true, 0);
  }
}
